// Forked in 2023 by VeederLicht in order to convert it to a callable C++ function
//
// ORIGINAL: 'spaminifier' Copyright (C) 2021 Antti Helminen (brebl)
...
int main(int argc, char **argv)
{
    ...
    {
        std::cout
            << "minifier Copyright (c) 2021 Antti Helminen (brebl@github)\n"
            << "spaminifier Copyright (c) 2023 VeederLicht (veederlicht@github)\n"
            << "Combine all web-assets into one minified html-file\n"
            << "--version: " << MAJOR_VERSION << "." << MINOR_VERSION << "." << PATCH_VERSION << "\n"
            << "--usage: spaminifier -i <html-source> [options]\n\n"
            << "Options:\n"
            ...
            << "Feedback:   https://github.com/VeederLicht/spaminifier\n";
        exit(0);
    }
    else
    {
#ifdef __GNUC__
        int opt = 0;
        while ((opt = getopt(argc, argv, "i:o:wtf")) != -1)
        {
            switch (opt)
            {
            // input filename
